...
def get_all_coords(images_path):
    ...
    for image_name in image_path_list:
        ...
        coords[image_name] = {}
        if "Latitude" in gps:
            coords[image_name]["lat"] = gps["Latitude"]
            coords[image_name]["lon"] = gps["Longitude"]
            coords[image_name]["alt"] = gps["Altitude"]
        else:
            coords[image_name]["lat"] = None
            coords[image_name]["lon"] = None
            coords[image_name]["alt"] = None

    return coords


def select_4NN_ours(image_name, coords):
    ...
    for i in range(0, len(neighbor_1)):
        ...
        dist1[p_num_ten] = cv_utils.get_gps_distance(lat_ref, lon_ref,
    ...
    for i in range(0, len(neighbor_2)):
        ...
        dist2[p_num_ten] = cv_utils.get_gps_distance(lat_ref, lon_ref,
                                                     coords[p_num_ten]["lat"],
                                                     coords[p_num_ten]["lon"])
    ...
    for i in range(0, len(neighbor_3)):
        ...
        dist3[p_num_ten] = cv_utils.get_gps_distance(lat_ref, lon_ref,
                                                     coords[p_num_ten]["lat"],
                                                     coords[p_num_ten]["lon"])
    ...
    for i in range(0, len(neighbor_4)):
        ...
        dist4[p_num_ten] = cv_utils.get_gps_distance(lat_ref, lon_ref,
                                                     coords[p_num_ten]["lat"],
                                                     coords[p_num_ten]["lon"])
...
def get_all_neighbors_4NN(images_path):
    matching_image_name_pair = []
    images_name = os.listdir(images_path)
    coords = get_all_coords(images_path)
    for image_name in images_name:
        neighbor_4NN = select_4NN_ours(image_name, coords)
...
def image_index(images_path, image_name):
    # return image index from image name
    img_path_list = os.listdir(images_path)
    img_index = 0
    for p_num in range(len(img_path_list)):
        if img_path_list[p_num] == image_name:
            img_index = p_num
            break
    return img_index
...
def matching_image_index_pair(images_path, matching_image_name_pair):
    ...
    for match_index in range(match_num):
        ...
        img1_index = image_index(images_path, img1_name)
        img2_name = matching_image_name_pair[match_index][1]
        img2_index = image_index(images_path, img2_name)
